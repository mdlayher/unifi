...
// Type returns the AP's DeviceType.
func (a *AP) Type() DeviceType {
	return DeviceTypeAP
}

// A Radio is a wireless radio, attached to an AP.
type Radio struct {
	BuiltInAntenna     bool
...
// APStats contains AP device network activity statistics.
type APStats struct {
	TotalBytes int64
	All        *WirelessStats
	Guest      *WirelessStats
...
func (d *AP) UnmarshalJSON(b []byte) error {
	var dev ap
	...
	*d = AP{
		...
		Serial:    dev.Serial,
		SiteID:    dev.SiteID,
		Uptime:    time.Duration(dev.Uptime) * time.Second,
		Version:   dev.Version,
		Stats: &APStats{
...
// An ap is the raw structure of a AP returned from the UniFi Controller
// API.
type ap struct {
